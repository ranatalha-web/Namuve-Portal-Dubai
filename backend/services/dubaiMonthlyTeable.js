 ...
 */
async function fetchAllDailyRecords(token, pageSize = 100) {
  const allRecords = [];
  let skip = 0;
  let hasMore = true;
...
async function fetchLatestDailyRevenue() {
  try {
    ...
    const allRecords = await fetchAllDailyRecords(finalToken);

    if (allRecords.length === 0) {
      throw new Error('No daily revenue records found');
    }

    console.log(`📊 Total daily records fetched: ${allRecords.length}`);
